...
app.get(['/index','/index/:id'], function(req, res){//메인페이지(id값을 통하여 글 내용을 볼 수 있음)
  var sql = "SELECT no, subject, contents, DATE_FORMAT(create_time, '%Y-%m-%d %H:%i:%s')date FROM pkt.pkt_data";
    conn.query(sql, function(err, topics, fields){
      var id = req.params.id; // request받은 id값
      if(id){// 글을 선택 했을때.
        var sql = "SELECT no, subject, contents, DATE_FORMAT(create_time, '%Y-%m-%d %H:%i:%s')date FROM pkt.pkt_data WHERE no=?";
        conn.query(sql, [id], function(err, topic, fields){//[id] : 사용자로부터 받은 id
          if(err) {
            console.log(err);
            res.status(500).send('Internal Server Error');
        ...
        res.render('index', {topics : topics })//topic의 데이터가 없어도 topic을 명시해 주지 않는다면 ejs가 오류를 낸다.
      }
    });
});
